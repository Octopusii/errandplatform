	...
	<view class="page">
		...
		<scroll-view scroll-y class="content" :show-scrollbar="false">
			...
			<view class="card">
				...
				<textarea class="item-textarea" v-model="itemDescription" placeholder="描述一下您想买的物品，如：一份双拼炸鸡、一盒感冒药等..." />
				<view class="tag-list">
			...
			<view class="price-row">
				<view class="card price-card">
					...
					<view class="price-input-row">
						...
						<input class="price-input" v-model="estimatedPrice" placeholder="0.00" type="digit" />
					</view>
				</view>
			...
			<!-- Delivery Address -->
			<view class="card delivery-card" @tap="openAddressModal">
				<view class="delivery-left">
					<view class="delivery-icon">🏠</view>
					<view>
						<text class="delivery-title">送到哪里？</text>
						<text class="delivery-hint">{{ deliveryAddress || '点击选择收货地址' }}</text>
					</view>
				</view>
			...
			<view class="card">
				...
				<input class="remark-input" v-model="remark" placeholder="给骑手带句话..." />
			</view>
			...
			<view class="fee-card">
				...
				<view class="fee-row total-row">
					...
					<text class="fee-value total-value">¥{{ deliveryFee }}</text>
				</view>
			</view>
		...
		<view v-if="showAddressModal" class="modal-overlay" @tap="cancelAddress">
			<view class="modal-card" @tap.stop>
				<text class="modal-title">收货地址</text>
				<input class="modal-input" v-model="tempAddress" placeholder="请输入收货地址" />
		...
		<view class="bottom-bar">
			<view class="bottom-left">
				...
				<text class="bottom-price">¥{{ deliveryFee }}</text>
	...
	methods: {
		...
		async goSuccess() {
			...
			if (!this.itemDescription) {
				uni.showToast({ title: '请填写想买什么', icon: 'none' })
				return
			}
			...
			try {
				...
				const res = await order.buyOrder({
					uid: getUid(),
					item_description: this.itemDescription,
					delivery_address: this.deliveryAddress,
					estimated_price: parseFloat(this.estimatedPrice) || 0,
					remark: this.remark,
					delivery_fee: this.deliveryFee,
					purchase_mode: this.addressMode
				})
